...
def checkTwoInRow(board, theirmove, other, player):
    ...
    for i in range(1, 5):
        if(row_num(i, board) - 1 == row_num(theirmove, board)):
            ...
            if(cmax == 3):
                for j in range(7):
                    if(row_num(j, board) != row_num(theirmove, board)):
...
def runseach(board, player, theirmove):
    ...
    if(board[rowLast].count(".") >= 5 and theirmove != 6 and theirmove != 0 and board[row_num(theirmove - 1, board)][theirmove - 1] == "." and row_num(theirmove + 1, board) == rowLast && board[rowLast][theirmove] == board[row_num(theirmove + 1, board)][theirmove + 1]):
        result = checkTwoInRow(board, theirmove, theirmove + 1, player)
        
    if(board[rowLast].count(".") >= 5 and theirmove != 6 and theirmove != 0 and board[row_num(theirmove + 1, board)][theirmove + 1] == "."row_num(theirmove - 1, board) == rowLast && board[rowLast][theirmove] == board[row_num(theirmove - 1, board)][theirmove - 1]):
        result = checkTwoInRow(board, theirmove, theirmove - 1, player)
        
        
        #row1 = row_num(i, board)
        #row2 = row_num(i + 1, board)
        
        #if(row1 != row2 and row1 != -1 and row2 != -1):
        #    continue
        
        #elif(board[row1][i] == board[row2][i + 1]):
            
            #if(row_num(i - 1, board) == row_num(i + 2, board) and row_num(i + 2, board) == row_num(i + 3, board)):
            #    return i + 2
            #if(row_num(i - 1, board) == row_num(i - 2, board) and row_num(i - 2, board) == row_num(i + 2, board)):
            #    return i - 1
    n = 0
